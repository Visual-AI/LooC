...
def get_sh(cfg):
    ...
    exp_name = f"{cfg['dataset']}_cos_closest_{cfg['embedding_num']}x{cfg['embedding_dim']}"
    str_list = []
    ##### train
    ...
    str_list.append("--distance cos \\")
    str_list.append("--num_workers 8 \\")
    str_list.append(f"--anchor closest 2>&1 | tee {output_folder}/{exp_name}_train.log")
    str_list.append(" ")
    ...
    if cfg['lora_codebook']:
        str_list.append("--lora_codebook \\")
    str_list.append(f"--distance cos  2>&1 | tee {output_folder}/{exp_name}_test.log")
    str_list.append(" ")
    ...
    sh_filename = f"scripts/run_{cfg['dataset']}_{cfg['embedding_num']}x{cfg['embedding_dim']}.sh"
